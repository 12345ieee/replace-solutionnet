...
def find_neighbours(curr, field):
...
def reorder_pipe(pipe, seed):
    ...
    other_seed = None
    for pt in pipe:
        if len(find_neighbours(pt, field)) == 1 and pt != seed:
            other_seed = pt
    ...
    if not other_seed:
        ...
        output = build_pipe(field, [seed], len(pipe), None, 2000)
        ...
        for i in range(6):
            out_forward = build_pipe(field, out_forward, len(pipe) - len(out_backward),
                                     out_backward[-1], 200, clean=True)
            out_backward = build_pipe(field, out_backward, len(pipe) - len(out_forward),
...
def build_pipe(field, starting_output, target_len, target_point=None, iterations=2000, clean=False) -> list:

    def is_neighbour(pt1: Point, pt2: Point) -> bool:
